  ...
  return {
    ...
    fontSize: isMobile
      ...
      : isTablet
        ? 16 + Math.random() * 8
        : 18 + Math.random() * 8,
    animationDuration: 6 + Math.random() * 4, // 6-10s para chuva devagar
...
function MatrixBackgroundInner({ variant = "global" }: MatrixBackgroundProps) {
  ...
  const positionClass =
    variant === "global" ? "fixed inset-0 -z-10" : "absolute inset-0";

  return (
    <>
      {/* Estilos da animação injetados inline para ser autossuficiente */}
      <style>{`
      ...
      >
        {matrixColumns.map((column, columnIndex) => {
          // Profundidade 3D por coluna: algumas mais "perto", outras mais "longe"
          const depthFactor = (columnIndex % 8) - 4; // -4 a 3 (mais variação)
          const translateZ = depthFactor * 80; // -320px a 240px (3D mais forte)
          const scale = 1 + depthFactor * -0.07; // diferença maior entre perto/longe
          const skewY = depthFactor * -3; // inclinação mais agressiva
