...
var FileUpload = React2__namespace.forwardRef(
  ({
    className,
    files = [],
    onFilesAdd,
    ...
    const processFiles = React2__namespace.useCallback((fileList) => {
    ...
    const handleDrop = React2__namespace.useCallback((e) => {
      e.preventDefault();
      setIsDragging(false);
    ...
    const handleDragOver = React2__namespace.useCallback((e) => {
      e.preventDefault();
      if (!disabled && !loading) {
        setIsDragging(true);
      }
    }, [disabled, loading]);
    const handleDragLeave = React2__namespace.useCallback((e) => {
      e.preventDefault();
      setIsDragging(false);
    }, []);
    const handleFileSelect = React2__namespace.useCallback((e) => {
    ...
    const handleRemoveFile = React2__namespace.useCallback((id) => {
      onFileRemove?.(id);
    }, [onFileRemove]);
