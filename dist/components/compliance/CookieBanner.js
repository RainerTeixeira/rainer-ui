...
var Separator = React2__namespace.default.forwardRef(
  ({ className, ...props }, ref) => /* @__PURE__ */ jsxRuntime.jsx(
    "div",
    {
      ref,
...
function CookieBanner() {
  const [showBanner, setShowBanner] = React2.useState(false);
  const [canShowBanner, setCanShowBanner] = React2.useState(false);
  const [showCustomize, setShowCustomize] = React2.useState(false);
  const [preferences, setPreferences] = React2.useState({
    essential: true,
  ...
  const hasShownBannerRef = React2.useRef(false);
  const scrollThresholdRef = React2.useRef(0);
  const scrollTimeoutRef = React2.useRef(null);
  const mouseTimeoutRef = React2.useRef(null);
  React2.useEffect(() => {
