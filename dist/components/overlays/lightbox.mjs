...
var Lightbox = React2.forwardRef(
  ({
    className,
    variant = "default",
    animation = "fade",
    ...
    const [zoom, setZoom] = React2.useState(ZOOM_CONFIG.DEFAULT);
    const [rotation, setRotation] = React2.useState(0);
    const [isPlaying, setIsPlaying] = React2.useState(autoPlayVideos);
    const [isFullscreen, setIsFullscreen] = React2.useState(false);
    const [touchStart, setTouchStart] = React2.useState(null);
    const [isDragging, setIsDragging] = React2.useState(false);
    const [dragOffset, setDragOffset] = React2.useState({ x: 0, y: 0 });
    const videoRef = React2.useRef(null);
    const containerRef = React2.useRef(null);
    const mediaRef = React2.useRef(null);
    ...
    const handleZoomIn = React2.useCallback(() => {
      setZoom((prev) => Math.min(prev + ZOOM_CONFIG.STEP, ZOOM_CONFIG.MAX));
    }, []);
    const handleZoomOut = React2.useCallback(() => {
      setZoom((prev) => Math.max(prev - ZOOM_CONFIG.STEP, ZOOM_CONFIG.MIN));
    }, []);
    const handleResetZoom = React2.useCallback(() => {
      setZoom(ZOOM_CONFIG.DEFAULT);
      setRotation(0);
      setDragOffset({ x: 0, y: 0 });
    }, []);
    const handleRotate = React2.useCallback(() => {
      setRotation((prev) => (prev + 90) % 360);
    }, []);
    const handlePlayPause = React2.useCallback(() => {
      if (videoRef.current) {
        if (isPlaying) {
    ...
    const handleToggleFullscreen = React2.useCallback(() => {
