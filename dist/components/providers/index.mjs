...
function TokensProvider({ tokens: customTokens, children }) {
  ...
  const getColor = React.useCallback((path, fallback = "#000000") => {
    return getNestedValue(tokens.primitives?.colors, path, fallback) || getNestedValue(tokens.semantics, path, fallback);
  }, [tokens]);
  const getSpacing = React.useCallback((key, fallback = "0") => {
    return tokens.primitives?.spacing?.[key] ?? fallback;
  }, [tokens]);
  const getRadius = React.useCallback((key, fallback = "0") => {
    return tokens.primitives?.radius?.[key] ?? fallback;
  }, [tokens]);
  const getShadow = React.useCallback((key, fallback = "none") => {
...
function UIProvider({
  ...
  if (!disableTokensProvider) {
    content = /* @__PURE__ */ jsx(TokensProvider, { tokens: resolvedTokens, children: content });
  }
...
export { DEFAULT_TOKENS, ThemeProvider, TokensContext, TokensProvider, UIProvider, useTokens };
